...
{
    // 簡單移動平均交叉策略
    public class 移動平均交叉策略(int 短期週期 = 5, int 長期週期 = 20) : 交易策略
    {
        public override 交易信號 產生信號(List<金融數據> 數據, int 當前索引)
        {
            if (當前索引 < 長期週期) return 交易信號.持有;

            var 短期MA = 金融分析器.計算移動平均(數據.Take(當前索引 + 1).ToList(), 短期週期);
            var 長期MA = 金融分析器.計算移動平均(數據.Take(當前索引 + 1).ToList(), 長期週期);
